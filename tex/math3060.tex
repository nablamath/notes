...
\begin{thm}
  ...
  $$\mathbf{x}(t)=\begin{cmatrix}
    x_{1}(t)\\
    \vdots\\
    x_{n}(t)
  \end{cmatrix}\in[x_{1}-b,x_{1}+b]\times\cdots\times[x_{n}-b,x_{n}+b]$$\s

  $$\mathbf{x}_{0}=\begin{cmatrix}
    x_{1}\\
    \vdots\\
    x_{n}
  \end{cmatrix},\mathbf{f}(t,x)=\begin{cmatrix}
    f_{1}(t,x)\\
    \vdots\\
    f_{n}(t,x)
  \end{cmatrix}\in C^{1}(R)$$\s
...
$$\frac{\diff\mathbf{x}}{\diff t}=\begin{cmatrix}
  \diff x/\diff t\\
  \diff^{2}x/\diff t^{2}\\
  \vdots\\
  \diff^{m}x/\diff t^{m}
\end{cmatrix}=\mathbf{f}(t,\mathbf{x})$$\s

with

$$\mathbf{x}(t_{0})=\begin{cmatrix}
  x_{0}\\
  x_{1}\\
  \vdots\\
  x_{m-1}
